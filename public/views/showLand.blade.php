...
<script>
  ...
  function makeBox(state,id){
    ...
    var block= '<label ><input type="checkBox" name="land_ids[]" id="land'+id+'" value="'+id+'"/>'+Icon+'</label>';
  ...
  function makeLand2(divID,states,landIDs){
    ...
    for(var i=0;i<tableHeight ;i++){
      ...
      for(var j=0; j<tableWidth && cellCount<t;j++){
        ...
        current_col.innerHTML=makeBox(state,landIDs[cellCount]);
        cellCount++;
      }
    }
  ...
  function updateLands(x){
    x= typeof(x)=='undefined'?"land1":x; //initial
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        makeLand2("Land",states,landIDs); //this removes previous land & replaces with new states
        //put this following on onclick () directly
