...
//we return that RGT
//clean or fert-only lands will have seed_id = -1 , for these, return GT = Config::get('game.maxGT');
//so return GT only for planted_seeds i.e. seed_id >0

	public function checkRGT($q,$planted_at){
	if($q==0)return Config::get('game.maxGT');//this will never make it fruit
	$GT= Config::get('game.seedGT') / $q;
