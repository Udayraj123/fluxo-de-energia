...
//for Test
	...
	public function calcStorageLE($fruit){
		$maxQual=Config::get('game.maxQual');
		$storage_fac=Config::get('game.storage_fac');
//add complexity here.
		return $fruit->unit_price * (1 + $storage_fac*$fruit->seed->product->quality/$maxQual);
	}

	public function calcSellPrice($fruit){
		$sellC1=Config::get('game.sellC1');
		$sellC2=Config::get('game.sellC2');
	...
	public function getUnitPrice($input){
		...
		$c3=Config::get('game.fruitC3');
		$c4=Config::get('game.fruitC4');
		$bp=Config::get('game.fruitBP');
	...
	public function launchFruit(){
		...
		if($f0->launched != 0 || $num_units==0)
			return Config::get('debug.dontFool');
...
//another ajax on click
	public function fetchFruit(){
		...
		if( $this->getRGT($l) < 0 ){
			...
			//recheck here
			$increase=Config::get('game.fruitReturns') * $fruit->seed->product->quality;
...
//this will be used only when planting seed. later, only RGT will be updated separately
	public function getGT($l){
		if($l->seed_id < 0 )return Config::get('game.maxGT');
...
//pass seed's quality, fert's quality, land's quality.
	if($lq*$sq*$fq==0)return Config::get('game.maxGT');//this will never make it fruit
...
//maybe put this into a function
		...
		//Notes this in purchases table
			...
			$pch->avl_units = $avl_units; //added on 8Oct, should have been since long!
