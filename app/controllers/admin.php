...
//create Product TODO - RFT's need to be updated with ajax along with the ones in makeInvestment. Else its too static for gods
...
Todo:: Now you have extracted 6 files to connect to backend, work upon them
Just list the correspondence here & work on it later-
	...
	Done - 	funding_list = makeInvestment & 
			Land_menu = showLand 
	Done - listInvestments = Investor's own shares' stats
...
-> : farmer's buyProduct shall check expiry !
lets go. Current LE = 41472. Buy price = -23968342.960167 Num= 10Success. Now LE = 479408331.20333
...
Low -
->Mostly done:: if($p->launched) shall be corrected to  $p->launched==1 since -1 is also getting here.
-> change this types to array_key_exists- if(!(array_key_exists('num_shares', $input) && array_key_exists('product_id', $input))) return "Input not read.";

//Make every created_at into Nullable default null - ELSE MAINTAIN A DIFF COLUMN - BEST
-> Prefer using Input::get('num_shares')); //return "input get is calm";
s
